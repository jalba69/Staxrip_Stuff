...
In case you managed to have a Gifski.exe version with **built in FFMPEG+Avisynth support** grab the script [here](https://github.com/jalba69/Staxrip_Stuff/tree/master/Gif_with_Gifski/Gifski_with_FFMPEG%2BAvisynth).  This version skips the step extracting the png's needed with the basic version from this current page, it still offers this possibility as option if you want to, but by default you will feed Gifski directly with the .avs input from Staxrip when using the script in link.
